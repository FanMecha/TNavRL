...
class RayCasterCamera(RayCaster):
    ...
    def _update_buffers_impl(self, env_ids: Sequence[int]):
        ...
        if "distance_to_image_plane" in self.cfg.data_types:
            ...
            frame = distance_to_image_plane.view(-1, *self.image_shape, 1)  # (N,H,W,1)
            self._data.output["distance_to_image_plane"][env_ids] = distance_to_image_plane.view(
                -1, *self.image_shape, 1
            )
            if self._hist_len > 0:
                self._data.history["distance_to_image_plane"][self._hist_idx, env_ids] = frame
        ...
        if self._hist_len > 0:
            ...
            if self._hist_idx == 0:
                self._hist_full = True

    def get_history(self, name: str) -> torch.Tensor:
        assert self._hist_len > 0 and hasattr(self._data, "history"), "history not enabled"
        hist = self._data.history[name]  # (L,N,H,W,C)
        if self._hist_full:
            return torch.cat([hist[self._hist_idx:], hist[:self._hist_idx]], dim=0)
        else:
            return hist[:self._hist_idx]
    ...
    def _create_buffers(self):
        ...
        self._hist_len = int(getattr(self.cfg, "history_length", 0) or 0)
        if self._hist_len > 0:
            self._data.history = {}
            for name in self.cfg.data_types:
                if name in ["distance_to_image_plane", "distance_to_camera"]:
                    ...
                    shape = (self.cfg.pattern_cfg.height, self.cfg.pattern_cfg.width, 3)
                self._data.history[name] = torch.zeros(
                    (self._hist_len, self._view.count, *shape), device=self._device
                )
            self._hist_idx = 0
            self._hist_full = False
