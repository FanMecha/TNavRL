...
def reset_root_state_usd_fixed_height(
        env: ManagerBasedEnv,
        env_ids: torch.Tensor,
        pose_range: dict[str, tuple[float, float]],
        velocity_range: dict[str, tuple[float, float]],
        asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
        # 以下参数对应你在 EventCfg 里的 params
        max_retries: int = 20,  # (缓存模式下不再使用，为了兼容 Config 保留)
    ...
    # --- 1. 获取缓存 ---
    # 扫描范围直接用 config 里的 pose_range
    scan_ranges = {
        "x": pose_range.get("x", (-2.0, 2.0)),
        "y": pose_range.get("y", (-2.0, 2.0))
    }

    # 这里的 robot_radius 对应你 Config 里的 0.7
    valid_points = MapScanner.get_or_create_cache(
        env,
        ranges=scan_ranges,
        step=0.5,  # 扫描间隔 0.5米
        check_radius=robot_radius,  # 碰撞半径
        height=fixed_height,  # 固定高度
        debug_name="robot_spawn"  # 缓存标识
    )

    # --- 2. 随机采样 (并行操作) ---
    num_reset = len(env_ids)
    # 随机生成索引
    indices = torch.randint(0, len(valid_points), (num_reset,), device=asset.device)
    selected_positions = valid_points[indices]

    # --- 3. 赋值 ---
    positions = torch.zeros((num_reset, 3), device=asset.device)
    positions[:, 0] = selected_positions[:, 0]
    positions[:, 1] = selected_positions[:, 1]
    positions[:, 2] = selected_positions[:, 2]

    # --- 姿态 (随机 Yaw) ---
    yaw_range = pose_range.get("yaw", (-3.14, 3.14))
    rand_yaw = (torch.rand(num_reset, device=asset.device) * (yaw_range[1] - yaw_range[0]) + yaw_range[0])
    zeros = torch.zeros_like(rand_yaw)
    orientations = math_utils.quat_from_euler_xyz(zeros, zeros, rand_yaw)

    # --- 速度 (清零) ---
    velocities = torch.zeros((num_reset, 6), device=asset.device)
