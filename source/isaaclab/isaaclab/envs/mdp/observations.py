...
def depth_image(env: ManagerBasedEnv, camera_name: str, near: float, far: float, Len: int = 1, viz: bool = False, viz_scale: int = 10) -> torch.Tensor:
    hist = env.scene.sensors[camera_name].get_history("distance_to_image_plane")  # (L_hist, B, H, W, 1)
    L_hist = hist.shape[0]
    if L_hist >= Len:
        hist_sel = hist[-Len:]
    else:
        pad = hist[:1].expand(Len - L_hist, *hist.shape[1:])
        hist_sel = torch.cat([pad, hist], dim=0)
    depth = hist_sel.permute(1, 0, 4, 2, 3).contiguous()    # (L, B, H, W, 1) → (B, L, 1, H, W)

    # 归一化到 [0,1]，并把无效/非正的深度置为 far 再归一化
    depth = torch.nan_to_num(depth, nan=0.0, posinf=0.0, neginf=0.0)
    invalid = depth <= 0
    depth = torch.clamp(depth, min=near, max=far)
    depth[invalid] = far
    depth_norm = (depth - near) / (far - near)  # (B, L, 1, H, W)

    if viz:
        latest = depth_norm[0, -1, 0].detach().float().cpu().numpy()  # (H, W), ∈[0,1]
        img8 = (np.clip(latest, 0, 1) * 255).astype(np.uint8)
        img8 = cv2.resize(img8, (img8.shape[1] * viz_scale, img8.shape[0] * viz_scale), interpolation=cv2.INTER_NEAREST)
        cv2.imshow(camera_name, img8)
        cv2.waitKey(1)
    return depth_norm.flatten(start_dim=1)


def goal_point(env: ManagerBasedEnv) -> torch.Tensor:
    """将目标位置转换到世界坐标系下的相对坐标 (goal - robot)，并提取机器人yaw角"""
    goal_pos = env.command_manager.get_command("pose_command")[:, :3]
    robot_pos = env.scene["robot"].data.root_pos_w.clone()
    robot_quat = env.scene["robot"].data.root_quat_w.clone()
    goal_world_rel = goal_pos - robot_pos
    qw, qx, qy, qz = robot_quat[:, 0], robot_quat[:, 1], robot_quat[:, 2], robot_quat[:, 3]
    robot_yaw = torch.atan2(2*(qw*qz + qx*qy), 1 - 2*(qy*qy + qz*qz))
    return torch.cat([goal_world_rel[:, :2], robot_yaw.unsqueeze(1)], dim=1)


def base_lin_xy(env: ManagerBasedEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
...
def base_ang_z(env: ManagerBasedEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
